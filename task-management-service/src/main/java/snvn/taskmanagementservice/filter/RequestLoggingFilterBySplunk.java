...
@ConditionalOnProperty(prefix = "task-management-service-log.splunk", name = "enabled", havingValue = "true")
public class RequestLoggingFilterBySplunk implements Filter {

    private static final Logger log = LoggerFactory.getLogger(RequestLoggingFilterBySplunk.class);

    private final ExternalLogService externalLogService;

    public RequestLoggingFilterBySplunk(ExternalLogService externalLogService) {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        externalLogService.sendLogSplunk("INFO",
        ...
        try {
            ...
            externalLogService.sendLogSplunk(level,
                    String.format("Completed request: %s %s status=%d duration=%dms", method, fullPath, status, duration),
                    responseContext);

            log.info("Request completed: {} {} status={} duration={}ms", method, fullPath, status, duration);
        }
    }
