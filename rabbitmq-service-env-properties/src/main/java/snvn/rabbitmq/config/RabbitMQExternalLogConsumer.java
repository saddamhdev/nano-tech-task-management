...
/**
 * Consumes JSON log events published by {@link RabbitMQExternalLogServiceImplementation}.
 * <p>
 * The queue, fanout exchange, and binding are declared by {@link LogExchangeConfig}.
 * This consumer simply listens on the queue bean created there.
 * <p>
 ...
 */
public class RabbitMQExternalLogConsumer {

    private static final Logger log = LoggerFactory.getLogger(RabbitMQExternalLogConsumer.class);
    ...
    public RabbitMQExternalLogConsumer(RabbitMQLogProperties properties,
    ...
    @RabbitListener(queues = "${rabbitmq-service-log.queue}", autoStartup = "${rabbitmq-service-log.auto-startup:true}")
        ...
        try {
            ...
            if (properties.isSplunkEnabled()) {
                ...
                CompletableFuture.runAsync(() -> {
                    if (restClient == null) {
                        log.info("[RabbitMQ-Splunk] RestClient not available. Consumed log event: {}", payload.get("event"));
                        return;
                    }
                    try {
                        restClient.post()
            ...
            if (properties.isLogfileEnabled()) {
                if (logFileService == null) {
                    log.warn("[RabbitMQ-Log] LogFileServiceImplementation is not available. Skipping log file write.");
                } else {
                    CompletableFuture.runAsync(() -> {
                        try {
                            System.out.println("[RabbitMQ-Log] LogFileServiceImplementation is available."+context);
    ...
    private void handleErrorEvent(Map<String, Object> event) {
        ...
        if (exception != null) {
            log.error("[RabbitMQ-Log] Exception detail: {}", exception);
        }
    }
